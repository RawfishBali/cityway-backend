class Admin::SchoolsController < Admin::BaseController
  ...
  def index
    if params[:school_type] && params.has_key?(:is_public)
      @admin_schools = City.find(session[:current_city_id]).utility.schools.where(school_type: params[:school_type], is_public: params[:is_public]).page(params[:page]).per(10)
    else
  ...
  def create
    ...
    respond_to do |format|
      if @admin_school.save
        format.html { redirect_to admin_schools_url, notice: 'Scuola è stato creato con successo.' }
  ...
  def update
    respond_to do |format|
      ...
      if @admin_school.update(admin_school_params)
        format.html { redirect_to admin_schools_url, notice: 'Scuola è stato aggiornato con successo.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_schools_url, notice: 'Scuola è stato distrutto con successo.' }
